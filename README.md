...
レパートリーページの表示内容は `data/repertoire.ts` で管理しています。

- 曜日を追加する場合は `repertoireByDay` 配列に `{ day, label, songs }` のまとまりを追加します。
- 曲を追加する場合は、対象曜日の `songs` 配列に `{ title, artist, note }` を追加します。
- `note` は任意です。不要な場合は省略できます。
- 曲を削除する場合は、対象曜日の `songs` 配列から該当する曲のまとまりを削除します。
- 表示順は `data/repertoire.ts` に書いた順番どおりです。曜日や曲の並びを変えたい場合は、配列内の順番を入れ替えます。
